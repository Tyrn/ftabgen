...
g_inc         = "Inc"     # Include target directory
g_src         = "Src"     # Source target directory
...
def generate_source_files():
    ...
    table_inc_body = (
        ...
        f"#ifndef {define}\n"
        f"#define {define}\n\n"
        f"#define {elem_type().upper()}_{g_name.upper()}(x) {g_name}[(x) & {domain_mask()}]\n"
        f"{elem_type()}_t {g_name}[];\n\n"
        f"#endif /* {define} */\n"
    )
    ...
    table_src_head = (
        ...
        f"{domain_and_range}\n"
        f'#include "{g_name}.h"\n\n'
        f"{elem_type()}_t {g_name}[{g_domain}] =\n"
        "{\n"
    )
...
def range_textbox_on_text_change(text):
    global g_range
    g_range = int_always(text)
    print(f"Range:  0..{g_range - 1}")

def name_textbox_on_text_change(text):
    global g_name
    g_name = text
    print(f'Name:   "{g_name}"')

def inc_textbox_on_text_change(text):
    global g_inc
    g_inc = text
    print(f'Inc:    "{g_inc}"')

def src_textbox_on_text_change(text):
    global g_src
    g_src = text
    print(f'Src:    "{g_src}"')
